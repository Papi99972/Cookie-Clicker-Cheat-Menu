javascript:(function(){
  ...
  menu.style.cursor = 'url("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADIAAAAoCAIAAADPYOIYAAAAmklEQVRIDbXBAQEAAAABIP6PzgpVusFmuFcWJQLzMzWg4eGmA9f4+6R8AA4Dcc4m5G4lzZPgj1z1B0Z3fWA6U5Ivcf42K9eFq3bgzqr2ECu5EzXfgAAAAAElFTkSuQmCC") 5 5, auto';
  ...
  // Create the close button (green X)
  ...
  gainCookiesButton.onmousedown = () => {
    gainCookiesButton.style.transform = 'translateY(4px)';
  };
  gainCookiesButton.onmouseup = () => {
    gainCookiesButton.style.transform = 'translateY(0)';
  };
  ...
  autoClickButton.onmousedown = () => {
    autoClickButton.style.transform = 'translateY(4px)';
  };
  autoClickButton.onmouseup = () => {
    autoClickButton.style.transform = 'translateY(0)';
  };
  ...
  // Make buildings appear free when bought
  for (let i = 0; i < game.buildings.length; i++) {
    const building = game.buildings[i];
    const originalPrice = building.getPrice();
    building.getPrice = () => {
      if (building.unlocked) {
        return originalPrice;
      } else {
        return 0; // Set price to 0 if the building hasn't been purchased yet
      }
    };
    // Monitor when the building is bought, then restore its price
    building.buy = (function(originalBuy) {
      return function() {
        originalBuy.apply(this);
        // After the building is bought, restore the original price
        building.getPrice = () => originalPrice;
      };
    })(building.buy);
  }
